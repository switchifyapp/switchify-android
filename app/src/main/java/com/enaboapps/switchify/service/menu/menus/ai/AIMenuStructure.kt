...
object AIMenuStructure {
    ...
    /**
     * Creates an enhanced AI menu item that uses visual context from screenshots
     */
    fun createVisualAIMenuItem(accessibilityService: SwitchifyAccessibilityService): MenuItem {
        return MenuItem(
            id = "ai_visual_suggestions",
            userProvidedText = "🔍 Smart AI Suggestions",
            isLinkToMenu = true,
            closeOnSelect = false
        ) {
            showVisualAIMenu(accessibilityService)
        }
    }

    /**
     * Shows the enhanced AI suggestions menu with visual context
     */
    private fun showVisualAIMenu(accessibilityService: SwitchifyAccessibilityService) {
        val loadingMenu = LoadingMenu(
            accessibilityService = accessibilityService,
            loadingTextResource = R.string.ai_menu_analyzing
        )
        ...
        CoroutineScope(Dispatchers.Main).launch {
            val aiSuggestions = loadVisualAISuggestions(accessibilityService)
    ...
    /**
     * Loads AI suggestions with visual context from screenshot
     */
    private suspend fun loadVisualAISuggestions(accessibilityService: SwitchifyAccessibilityService): List<MenuItem> {
        Log.d(TAG, "Loading visual AI suggestions...")

        return try {
            val currentNodes = SystemNodeHolder.getNodes()
            
            if (currentNodes.isEmpty()) {
                Log.d(TAG, "No nodes available for visual AI analysis")
                return createEmptyStateMenuItem()
            }

            Log.d(TAG, "Taking screenshot for visual AI analysis...")
            
            // Take a screenshot for visual context
            var screenshot: android.graphics.Bitmap? = null
            var screenshotError: String? = null
            
            // Use a coroutine to capture screenshot synchronously
            val screenshotTask = kotlinx.coroutines.CompletableDeferred<Unit>()
            
            ScreenshotManager.takeScreenshotWithDelay(
                accessibilityService = accessibilityService,
                context = accessibilityService.applicationContext,
                delayMs = 100, // Quick screenshot
                saveToGallery = false,
                callback = object : ScreenshotManager.ScreenshotCallback {
                    override fun onScreenshotTaken(bitmap: android.graphics.Bitmap, timestamp: Long) {
                        screenshot = bitmap
            ...
            val capturedScreenshot = screenshot
            if (capturedScreenshot == null) {
                Log.w(TAG, "Failed to capture screenshot: $screenshotError, falling back to text-only AI")
                return loadAISuggestions(accessibilityService)
            }

            Log.d(TAG, "Screenshot captured, analyzing ${currentNodes.size} nodes with visual context...")

            // Use visual context for ranking
            val rankedNodes = AINodeRanker.rankNodesWithVisualContext(
                nodes = currentNodes,
                screenshot = capturedScreenshot,
                additionalContext = "Current screen in accessibility app - prioritize elements for users with motor disabilities"
            )
            
            if (rankedNodes.isEmpty()) {
                Log.d(TAG, "No ranked nodes returned from visual AI analysis")
                return createNoSuggestionsMenuItem()
            }

            Log.d(TAG, "Creating visual AI menu items for ${rankedNodes.size} ranked nodes")

            // Convert ranked nodes to menu items with enhanced labeling
            val menuItems = rankedNodes.take(MAX_SUGGESTIONS).mapIndexed { index, rankedNode ->
                createVisualNodeMenuItem(rankedNode, index + 1)
            }

            return menuItems

        } catch (e: Exception) {
            Log.e(TAG, "Error loading visual AI suggestions", e)
    ...
    /**
     * Creates a menu item for a visually-analyzed ranked node
     */
    private fun createVisualNodeMenuItem(
        rankedNode: AINodeRanker.RankedNode,
        index: Int
    ): MenuItem {
        val node = rankedNode.node
        ...
        val displayText = if (contentDescription.length > 25) {
            contentDescription.take(22) + "..."
        } else {
            contentDescription
        }

        // Enhanced priority indicators based on visual analysis
        val priorityText = when {
            rankedNode.score >= 90 -> "🔥 $displayText"  // Very high priority (visually prominent)
            rankedNode.score >= 80 -> "⭐ $displayText"  // High priority
            rankedNode.score >= 60 -> "✨ $displayText"  // Medium-high priority
        ...
        return MenuItem(
            id = "visual_ai_suggestion_$index",
            userProvidedText = priorityText,
            closeOnSelect = true
        ) {
            tapAtNode(node)
    ...
    /**
     * Demonstrates screenshot analysis capabilities
     */
    fun createScreenshotAnalysisMenuItem(accessibilityService: SwitchifyAccessibilityService): MenuItem {
        return MenuItem(
            id = "screenshot_analysis",
            userProvidedText = "📸 Analyze Current Screen",
            closeOnSelect = true
        ) {
            analyzeCurrentScreen(accessibilityService)
        }
    }

    /**
     * Analyzes the current screen with AI and shows results
     */
    private fun analyzeCurrentScreen(accessibilityService: SwitchifyAccessibilityService) {
        Log.d(TAG, "Starting screen analysis...")
        
        val aiManager = FirebaseAIManager()
        
        ScreenshotManager.takeScreenshotWithAIAnalysis(
            accessibilityService = accessibilityService,
            context = accessibilityService.applicationContext,
            aiManager = aiManager,
            delayMs = 1000,
            saveToGallery = false,
            callback = object : ScreenshotManager.AIScreenshotCallback {
                override fun onScreenshotTaken(bitmap: android.graphics.Bitmap, timestamp: Long) {
                    Log.d(TAG, "Screenshot taken for analysis")
                }
                ...
                override fun onScreenshotAnalysisStarted() {
                    Log.d(TAG, "AI analysis started...")
                }
                
                override fun onScreenshotAnalysisComplete(response: com.enaboapps.switchify.service.ai.AIResponse) {
                    Log.d(TAG, "AI analysis complete!")
                    if (response.isSuccess && response.content != null) {
                        Log.i(TAG, "AI Analysis Result: ${response.content}")
                        // In a real implementation, you might show this in a dialog or menu
                    }
                }
                
                override fun onScreenshotAnalysisFailed(error: String) {
