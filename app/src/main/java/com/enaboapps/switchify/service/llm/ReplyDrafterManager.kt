...
import com.enaboapps.switchify.screens.replydrafter.ReplyDrafterActivity
...
object ReplyDrafterManager {
    ...
    /**
     * Capture the conversation, then open [ReplyDrafterActivity] to draft the
     * replies. The activity is a visible, foreground screen — AICore (Gemini
     * Nano) only runs inference while the app is the top foreground app.
     */
        ...
        ScreenshotManager.takeScreenshotWithDelay(
            ...
            callback = object : ScreenshotManager.ScreenshotCallback {
                override fun onScreenshotTaken(bitmap: Bitmap, timestamp: Long) {
                    ...
                    ReplyDrafterScreenshotHolder.set(image)
                    service.startActivity(
                        Intent(service, ReplyDrafterActivity::class.java).addFlags(
