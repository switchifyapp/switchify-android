...
object ReviewPrompter {
    ...
    fun shouldPrompt(context: Context, cooldownMs: Long = DEFAULT_COOLDOWN_MS, now: Long = System.currentTimeMillis()): Boolean {
    ...
    fun recordPrompt(context: Context, now: Long = System.currentTimeMillis()) {
        PreferenceManager(context).setLongValue(PreferenceManager.PREFERENCE_KEY_REVIEW_LAST_SHOWN, now)
    }

    fun requestIfDue(context: Context, reviewManager: ReviewManager, cooldownMs: Long = DEFAULT_COOLDOWN_MS) {
